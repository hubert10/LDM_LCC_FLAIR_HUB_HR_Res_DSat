...
from models.autoencoder.autoencoder import  DiagonalGaussianDistribution
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.first_stage_model = AutoencoderKL.from_pretrained(
            hparams["diff_sat_weights"], subfolder="vae", sample_size=16, revision=None
        )
        ...
        with torch.no_grad():
            ...
            new_decoder_conv.weight[:3, :, :, :] = decoder_conv.weight  # output channels dimension
    ...
    def apply_model(
        ...
        x_recon = self.denoise_net(x_noisy, t, cond_txt, mtd, down_block_additional_residuals=cond)
    ...
    def p_losses(self, x_start, t, cond, x, img_lr, img_lr_up, mtd, closest_idx, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        for tps in range(cond_control[0].shape[1]):
            ...
            model_output = self.apply_model(
                x_noisy, t, cond_txt, [feat[:, tps] for feat in cond_control], mtd
            )
            # model_output: torch.Size([2, 4, 8, 8])
        ...
        # Radiometric, Temporal, Gradient and Spectral_loss
        aux_loss = (
    ...
    def forward(
        ...
        **kwargs,
        ...
        x_res = [
            self.img2res(x, img_lr_up[:, i]) for i in range(img_lr_up.shape[1])
        ]
        x_start = torch.stack(x_res, dim=1)

        x_start_res = self.closest_lr_sits_aer(x_start, closest_idx)

        encoder_posterior = self.encode_first_stage(x_start_res).latent_dist # from  DiffusionSat
        ...
        loss, model_outputs = self.p_losses(
            z, t, cond_total, x, img_lr, img_lr_up, mtd, closest_idx, *args, **kwargs
        )
        losses = {"sr": loss}
    ...
    def sample(
        ...
        latent_shape = (shape[0], shape[1], shape[2] // 8, shape[3] // 8)
