...
class SatUNet(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    The model is conditioned on SITS images and metadata
    ...
    def __init__(
        ...
        # Build control projections if control channels are provided
        self.control_projs = nn.ModuleList(
            [
                nn.Conv2d(c, unet_c, kernel_size=1)
                for c, unet_c in zip(
                    control_channels, layer_channels
                )
            ]
        )
        for p in self.control_projs.parameters():
        ...
        self.hr_projs = nn.ModuleList(
            [
                nn.Conv2d(c, unet_c, kernel_size=1)
                for c, unet_c in zip(
                    list(map(lambda x: x * 2, control_channels)),
                    layer_channels,
                )
            ]
        )
    ...
    def _feature_size_per_input_block(self):
        ...
        Used to configure the control_projs conv layers.
        """
        sizes = []
        ch = self.in_channels
    ...
    def forward(
        ...
        if down_block_additional_residuals is not None:
            ...
            for i, down_block_res_sample in enumerate(down_block_res_samples):
                ...
                idx = (i - 2) // 3
                # The inputs LR conditioning features are too small
                if 2 <= i <= 5 and (i - 2) % 3 == 0 and idx < len(down_block_additional_residuals):
                    residual = self.control_projs[idx](down_block_additional_residuals[idx])
                    residual = self.super_patch_crop_and_upsample(residual, down_block_res_sample)               
